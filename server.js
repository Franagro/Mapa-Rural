...
// ---------- CONSULTA DE PARCELA EN CATASTRO ARBA (proxy, evita CORS) ----------
const ARBA_WFS = 'https://geo.arba.gov.ar/geoserver/idera/wfs';

async function tryArba(url) {
  ...
  try {
    ...
    if (!txt.trim().startsWith('{')) return null;   // vino XML de error
    const j = JSON.parse(txt);
    if (j && Array.isArray(j.features) && j.features.length) return j;
    return null;
  } catch (e) {
    clearTimeout(t);
    return null;
...
app.get('/api/parcela', requireAuth, async (req, res) => {
  const lat = parseFloat(req.query.lat), lng = parseFloat(req.query.lng);
  if (!isFinite(lat) || !isFinite(lng)) return res.status(400).json({ error: 'Coordenadas inválidas' });

  // El nombre de la columna geometrica y la version varian segun el servidor:
  // probamos las combinaciones mas comunes hasta que una responda.
  const geomCols = ['geom', 'the_geom', 'shape', 'geometria'];
  const versions = ['2.0.0', '1.1.0'];
  for (const ver of versions) {
    const nameParam = ver === '2.0.0' ? 'typeNames' : 'typeName';
    const countParam = ver === '2.0.0' ? 'count' : 'maxFeatures';
    for (const gc of geomCols) {
      const params = new URLSearchParams({
        service: 'WFS', version: ver, request: 'GetFeature',
        [nameParam]: 'idera:Parcela',
        outputFormat: 'application/json',
        srsName: 'EPSG:4326',
        [countParam]: '1',
        cql_filter: `INTERSECTS(${gc}, POINT(${lng} ${lat}))`
      });
      const j = await tryArba(`${ARBA_WFS}?${params.toString()}`);
      if (j) return res.json(j.features[0]);
    }
  }
  res.status(404).json({ error: 'No se pudo obtener la parcela del catastro en este punto' });
});
