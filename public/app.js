...
function initMap() {
  ...
  map.on(L.Draw.Event.CREATED, (e) => {
    ...
    try {
      ...
      console.error('openDrawForm', err);
...
// Superficie en m2 sin depender de librerias externas (formula esferica).
function polygonAreaM2(geojson) {
  const R = 6378137;
  ...
  let area = 0;
  if (g.type === 'Polygon') {
    g.coordinates.forEach((r, i) => { area += (i === 0 ? 1 : -1) * ringArea(r); });
  } else if (g.type === 'MultiPolygon') {
    g.coordinates.forEach(poly => poly.forEach((r, i) => { area += (i === 0 ? 1 : -1) * ringArea(r); }));
  }
...
function openDrawForm(layer) {
  ...
  let areaHa = '0.00';
  try { areaHa = (polygonAreaM2(geojson) / 10000).toFixed(2); }
  catch (err) { console.error('area', err); }
  $('#areaDisplay').textContent = areaHa;

  let partidoName = '';
  try {
    if (partidosLayer && window.turf) {
      partidosLayer.eachLayer(pl => {
        try {
          if (turf.booleanIntersects(geojson, pl.toGeoJSON())) partidoName = pl.feature.properties.nombre;
        } catch (e) {}
      });
    }
  } catch (err) { console.error('partido', err); }
  $('#partidoDisplay').textContent = partidoName ? `Partido: ${partidoName}` : '';

  try {
    editingFieldId = null;
    ['#fieldName', '#fieldCrop', '#fieldCampaign', '#fieldStatus', '#fieldStatus1', '#fieldStatus2', '#fieldStatus3']
      .forEach(sel => { const el = $(sel); if (el) el.value = ''; });
    selectedColor = '#52C22F';
    const cc = $('#fieldColorCustom'); if (cc) cc.value = selectedColor;
    updateColorSwatchSelection();
    fillClientSelect();
  } catch (err) { console.error('form init', err); }
...
function switchTab(name) {
  ...
  // si hay un poligono dibujado sin guardar, que el formulario siga disponible
  if (name === 'draw' && pendingLayer) $('#drawForm').classList.remove('hidden');
}
